...
def generate_variants_task(
    ...
    """生成视频变体任务 - 支持 Animated Caption + 并行生成
    ...
    # 12种可用的词级动画模板（与 Remotion WordAnimation.tsx 一致）
    ANIMATION_TEMPLATES = [
        'minimalist',      # 极简风格 - 白色无背景
        'default',         # 默认风格 - 经典字幕效果
        'classic',         # 经典风格 - 衬线字体
        'neo_minimal',     # 新极简风格
        'hype',            # 动感风格 - 橙色高亮
        ...
        'fast',            # 快速风格 - Impact字体
        'vibrant',         # 活力风格 - 渐变背景
        'word_focus',      # 词焦点风格 - 当前词高亮
        'line_focus',      # 行焦点风格 - 整行高亮
        'retro_gaming',    # 复古游戏风格 - 像素风
        'model',           # 模特风格 - 高端斜体
    ]
    
    if enable_subtitle:
        # 随机选择一个模板
        selected_template = animation_template or random.choice(ANIMATION_TEMPLATES)
        logger.info(f"[Animated Caption] 使用模板: {selected_template}")
