...
async def download_variants(
    ...
    """Download all variants as ZIP (optimized: ZIP_STORED + chunked streaming)"""
    ...
    # 计算总大小（用于 Content-Length）
    total_size = sum(os.path.getsize(fp) for fp, _ in files)
    # ZIP 头/尾开销约 30 bytes/文件 + 22 bytes
    zip_overhead = len(files) * 30 + 22 + len(files) * 256  # 文件名开销
    estimated_zip_size = total_size + zip_overhead
    ...
    return StreamingResponse(
        ...
        headers={
            'Content-Disposition': f'attachment; filename="variants_{video_id}.zip"',
            'Content-Length': str(estimated_zip_size)
        }
    )
